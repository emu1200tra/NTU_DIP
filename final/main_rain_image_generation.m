...
% Add your path
addpath(genpath('C:\Users\user\Documents\DIP\rain transfer\test')); 

for iteration = 1:15
    % Target 'non-rain' image file
    %infilename = 'nonrain_1.tif'; 
    infilename = ['./test/r_color_output' , int2str(iteration) , '.jpg']; 
    % Exemplar 'rain' image file
    filename_mask = ['./test/' ,int2str(iteration) , 'rmask.jpg'];
    filename_image = ['./test/' ,int2str(iteration) ,'r.jpg'];
    ...
    figure;
    subplot(1,3,1);
    imshow(final_image,[0 255]);
    title('Synthetic rain image');
    subplot(1,3,3);
    imshow(input_image,[]);
    title('Rain image');
    %}
    imwrite( uint8(final_image),['./test/r_output', int2str(iteration) , '.jpg']);
    ...
    final_image = yuv2rgb(synthetic_rain_image , U , V);
    imwrite( uint8(final_image),['./test/r_output', int2str(iteration) , 'light.jpg']);
end
%{
subplot(1,3,2);
imshow(final_image,[0 255]);
